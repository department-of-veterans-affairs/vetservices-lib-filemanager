...
public class ByteArrayValidatorTest {
	...
	public final void testValidate() {
		...
		assertTrue(LibFileManagerMessageKeys.UNEXPECTED_ERROR.equals(messages.get(0).getKey()));

		arg = new ImplArgDto<>(bytesEmpty);
		messages = byteArrayValidator.validate(arg);
		assertNotNull(messages);
		assertTrue(LibFileManagerMessageKeys.FILE_BYTES_NULL_OR_EMPTY.equals(messages.get(0).getKey()));
		...
		messages = byteArrayValidator.validate(arg);
		assertNotNull(messages);
		assertTrue(LibFileManagerMessageKeys.FILE_BYTES_SIZE.equals(messages.get(0).getKey()));
	}
